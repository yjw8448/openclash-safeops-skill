...
| `templates/ffani-redirhost-smartdns-overlay.yaml` | Safe overlay for applying an FFAni-style Redir-Host + SmartDNS profile to exactly one existing config. | User asks to modify a current config such as `config-a(2).yaml` according to the FFAni template. | Apply only through `openclash_template_apply.py`; requires SmartDNS port audit and user approval before overwrite. |
