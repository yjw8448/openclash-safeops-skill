# OpenClash SafeOps — AI Agent Skill for Safe OpenWrt Router Management
...
5. **Single-Config Template Application** — Modifies exactly one target YAML (e.g., `pqjc(2).yaml`). Never touches other YAML files or merges subscriptions. Full pipeline: audit → guard → candidate → validate → approve → write.
...
├── SKILL.md
├── scripts/
│   ├── openclash_diagnose.sh
│   ├── openclash_backup.sh
│   ├── openclash_emergency_restore.sh
│   ├── openclash_watchdog.sh
│   ├── openclash_dns_audit.sh
│   ├── openclash_subscription_health.sh
│   ├── openclash_multisub_audit.sh
│   ├── openclash_multisub_guard.sh
│   ├── openclash_single_config_template_guard.sh
│   ├── openclash_template_apply.py
│   ├── openclash_lint_config.py
│   ├── openclash_group_detect.py
│   └── ...
├── templates/
│   ├── aethersailor-current-safe-overlay.yaml
│   ├── aethersailor-legacy-safe-overlay.yaml
│   ├── ffani-redirhost-smartdns-overlay.yaml
│   ├── overwrite-safe-basic.yaml
│   ├── overwrite-ai-dev.yaml
│   └── ...
├── references/
│   ├── document-index.md
...
│   ├── template-apply.md
│   ├── aethersailor-current-safe.md
│   ├── aethersailor-source-snapshot.md
│   └── ...
├── docs/
│   └── kb/
│       ├── 60-dns-decision-tree.md
│       ├── 70-subscription-decision-tree.md
│       ├── 75-multi-subscription-decision-tree.md
│       ├── 76-unbound-config-decision-tree.md
│       ├── 77-single-config-template-apply.md
│       └── ...
└── examples/
    └── usage-examples.md
```
...
I_UNDERSTAND_TARGETED_WRITE=1 python3 scripts/openclash_template_apply.py \
  --target "/etc/openclash/config/pqjc(2).yaml" \
  --template aethersailor-current-safe \
  --candidate /tmp/pqjc2.aethersailor-current-safe.candidate.yaml \
  --apply
```
...
一个面向 AI Agent 的技能/插件，通过 SSH 安全诊断、修复和配置 OpenWrt 路由器上的 OpenClash。强制只读诊断优先、写入前必须备份、绝不触碰 network/dhcp/firewall 系统配置，防止路由器锁死。
...
支持只针对一个目标 YAML 生成候选配置，例如：

```
/etc/openclash/config/pqjc(2).yaml
```

不会修改其他 YAML，不会合并其他订阅。
...
│   ├── aethersailor-current-safe.md  # Aethersailor Current-Safe 适配器
│   ├── aethersailor-source-snapshot.md # Aethersailor 源码快照
│   └── ...
├── docs/
│   └── kb/                           # 知识库（75+ 文档）
...
│       ├── 75-multi-subscription-decision-tree.md # 多订阅决策树
│       ├── 76-unbound-config-decision-tree.md    # 未绑定配置决策树
│       ├── 77-single-config-template-apply.md    # 单配置模板应用
│       ├── 79-aethersailor-current-safe-config-generation.md
│       ├── playbooks/                # 修复剧本
│       └── checklists/               # 检查清单
└── examples/
...
> 3. 不修改其他 YAML。
> 4. 不合并多个订阅。
> 5. 不修改 network/dhcp/firewall。
> 6. 不重启 network。
> 7. 不直接覆盖原文件。
> 8. 先生成 candidate，校验后等待我确认。
